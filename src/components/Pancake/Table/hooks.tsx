        ...
        case "TOGGLE_SORT":
          ...
          columnCopy = state.columns.map((column) => {
            ...
            if (action.columnName === column.name) {
              ...
              if (column.sort) {
                if (column.label.toUpperCase() === "APY") {
                  // const _data = state.rows.map(e => e?.original?.apy?.originalValue).filter(e => Boolean)
                  // @ts-ignore
                  sortedRows = isAscending ? orderBy(state.rows, (e) => e?.original?.apy?.originalValue) : orderBy(state.rows, (e) => e?.original?.apy?.originalValue).reverse();
                } else {
